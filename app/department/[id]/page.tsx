    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 md:py-12 font-sans">
      ...
      {products.length === 0 ? (
        <div className="text-center py-20 bg-surface-subtle rounded-3xl border border-line shadow-sm">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-full bg-surface-card text-steel mb-3 text-xl border border-line">
            📦
          </div>
          ...
          {products.map((p: any) => (
            ...
            >
              <div>
                ...
                <div className="relative w-full aspect-square bg-white rounded-xl overflow-hidden mb-3 flex items-center justify-center border border-line/40">
                  {p.images?.[0] ? (
                    <Image
                      src={p.images[0]}
                      alt={p.name}
                  ...
                  {p.discount && (
                    <span className="absolute top-2 left-2 bg-signal text-white font-black text-[10px] px-2 py-0.5 rounded shadow-sm">
                      -{p.discount}%
                    </span>
                  )}
                </div>
              ...
              <div className="mt-3 pt-2.5 border-t border-line flex items-center justify-between">
                ...
                <div className="w-7 h-7 rounded-full bg-surface-card group-hover:bg-signal text-steel group-hover:text-white flex items-center justify-center transition-colors text-xs border border-line group-hover:border-signal">
                  ➔
                </div>
              </div>
            </Link>
